...
//        product.getParts().addAll(assparts);
    ...
    public AddFurnitureController(PartService partService) {
...
// make the add and remove buttons work
 ...
 //        return "confirmationassocpart";
    ...
    @GetMapping("/buyFurniture")
    public String buyFurniture(@RequestParam("productID") int theId, Model theModel) {
        ProductService productService = context.getBean(ProductServiceImpl.class);
        Product product2=productService.findById(theId);
        ...
        if (inv == 0){
            ...
            product2.setInv(inv);
            //save product object with new inventory
            productService.save(product2);
